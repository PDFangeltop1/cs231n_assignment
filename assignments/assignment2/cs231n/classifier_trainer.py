...
class ClassifierTrainer(object):
  ...
  def train(self, X, y, X_val, y_val, 
            model, loss_function, 
            reg=0.0, dropout=None,maxout=None,bn=None,
    ...
    ###### restore paramter for Batch Normalization, such as bb2,rr2,bb3,rr3
    tmp_model = {}
    X_train_epoch = []
    ########################################################################
    if bn is not None:
    ...
    for it in xrange(num_iters):
      if it % 1000 == 0:  print 'starting iteration ', it
      ...
      # For each epoch ,record all batches to evaluate mean and var of BatchNormalization.   
      if bn is not None:
        X_train_epoch.append(X_batch)
        
      # evaluate cost and gradient
      cost, grads = loss_function(X_batch, model, y_batch, reg=reg, dropout=dropout,maxout=maxout,bn=bn)
      ...
      if first_it or epoch_end or acc_check:
        ...
        # so we batch it
        if bn is None:                    
          if X_train_subset.shape[0] < 100:
            scores = loss_function(X_train_subset,model)
            y_pred_train = np.argmax(scores,axis=1)
            ...
            for i in xrange(X_train_subset.shape[0] / 100):
              ...
              scores = loss_function(X_train_slice, model,dropout=dropout,maxout=maxout,bn=bn)
          ...
          average_mean2 = np.zeros((len(X_train_epoch),model['rr2'].shape[0]))
          average_var2 = np.zeros(average_mean2.shape)
          average_mean3 = np.zeros((len(X_train_epoch),model['rr3'].shape[0]))
          average_var3 = np.zeros(average_mean3.shape)
          for i,batch in enumerate(X_train_epoch):
            average_mean2[i], average_var2[i], average_mean3[i], average_var3[i] = loss_function(batch,model,maxout=maxout,bn="Average")
          
          Emean2 = np.mean(average_mean2,axis=0)
          Evar2 = np.mean(average_var2,axis=0)*batch_size/(batch_size-1)
          Emean3 = np.mean(average_mean3,axis=0)
          Evar3 = np.mean(average_var3,axis=0)*batch_size/(batch_size-1)
          
          tmp_model['rr2'] = model['rr2']
          tmp_model['rr3'] = model['rr3']
          tmp_model['bb2'] = model['bb2']
          tmp_model['bb3'] = model['bb3']
          
          model['rr2'] = tmp_model['rr2']/np.sqrt(Evar2+1e-8)
          model['bb2'] = tmp_model['bb2'] - (tmp_model['rr2']*Emean2)/np.sqrt(Evar2+1e-8)
          model['rr3'] = tmp_model['rr3']/np.sqrt(Evar3+1e-8)
          model['bb3'] = tmp_model['bb3'] - (tmp_model['rr3']*Emean3)/np.sqrt(Evar3+1e-8)
        ...
        if X_val.shape[0] < 100:
          ...
          for i in xrange(X_val.shape[0] / 100):
            ...
            scores = loss_function(X_val_slice, model,maxout=maxout,bn=bn)
        ...
        if bn is not None:
          model['rr2'] = tmp_model['rr2'] 
          model['rr3'] = tmp_model['rr3'] 
          model['bb2'] = tmp_model['bb2'] 
          model['bb3'] = tmp_model['bb3']
        ...
        if verbose:
          if bn is None:
            ...
            print ('Finished epoch %d / %d: cost %f, val %f, lr %e'
                 % (epoch, num_epochs, cost, val_acc, learning_rate))
