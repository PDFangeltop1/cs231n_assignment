...
def init_three_layer_convnet(input_shape=(3, 32, 32), num_classes=10,
                            filter_size=5, num_filters=(32, 128),
                             weight_scale=1e-2, bias_scale=0, dtype=np.float32,maxout=None,bn=None):
  """
  ...
  """
  maxout replace affine-relu-forward/affine-forward layer with maxout-forward layer
  """
  if maxout is not None:
    print "use maxout activation function!!"
    model['W2'] = np.random.randn(maxout,H * W * F1 / 4, FC)
    model['b2'] = np.random.randn(maxout,FC)
    model['W3'] = np.random.randn(maxout,FC, num_classes)
    model['b3'] = np.random.randn(maxout,num_classes)
  else:
  ...
  if bn is not None:
    model['rr2'] = np.random.randn(FC)*weight_scale
    model['bb2'] = np.random.randn(FC)
    model['rr3'] = np.random.randn(num_classes)*weight_scale
    model['bb3'] = np.random.randn(num_classes)
...
def three_layer_convnet(X, model, y=None, reg=0.0, dropout=None, maxout=None,bn=None):
  """
  ...
  - maxout: maxout replace affine-relu-forward/affine-forward layer with maxout-forward layer
  """
  ...
  """
  conv_maxout_pool_forward may not be a good idea.
  since maxout parameter will be too many. K*D*H
  maxout and dropout out are set together!.
  """
  a1, cache1 = conv_relu_pool_forward(X, W1, b1, conv_param, pool_param)
  ...
  if dropout is None and maxout is None:
    if bn is not None:
      rr2 = model['rr2']
      bb2 = model['bb2']
      rr3 = model['rr3']
      ...
      if bn == "Average":
        # average bn paramters from multiple batches
        x2,_ = affine_forward(a1,W2,b2)
        a2,cache2 = affine_bn_relu_forward(a1,W2,b2,rr2,bb2)
        x3,_ = affine_forward(a2,W3,b3)
        
        mean2 = np.mean(x2,axis=0)
        var2 = np.var(x2,axis=0)
        mean3 = np.mean(x3,axis=0)
        var3 = np.var(x3,axis=0)
        return mean2,var2,mean3,var3

      else:
        #perform regular parameter learning
        a2,cache2 = affine_bn_relu_forward(a1,W2,b2,rr2,bb2)
        scores, cache4 = affine_bn_forward(a2,W3,b3,rr3,bb3)
    else:
      a2, cache2 = affine_relu_forward(a1, W2, b2)
      scores, cache4 = affine_forward(a2, W3, b3)
  else:
    a2, cache2 = maxout_forward(a1,W2,b2) 
    d2, cache3 = dropout_forward(a2, dropout_param)
    scores, cache4 = maxout_forward(d2,W3,b3) 
  ...
  if dropout is None and maxout is None:
    if bn is not None:
      da2, dW3, db3, drr3, dbb3 = affine_bn_backward(dscores,cache4)
      da1, dW2, db2, drr2, dbb2 = affine_bn_relu_backward(da2,cache2)      
    else:
      da2, dW3,db3 = affine_backward(dscores,cache4)
      da1, dW2, db2 = affine_relu_backward(da2,cache2)
  else:
    dd2, dW3,db3 = maxout_backward(dscores,cache4)
    da2  = dropout_backward(dd2,cache3)
    da1, dW2, db2 = maxout_backward(da2,cache2)
...
def three_layer_convnet_old(X, model, y=None, reg=0.0, dropout=None, maxout=None):
  """
  Compute the loss and gradient for a simple three layer ConvNet that uses
  the following architecture:
...
def five_layer_convnet(X, model, y=None, reg=0.0, dropout=1.0,
                       ...
                       return_probs=False,maxout=None,bn=None):
  """
