...
def three_layer_convnet(X, model, y=None, reg=0.0):
  ...
  dW2 += reg*W2
  dW1 += reg*W1
  reg_loss = 0.5*reg*np.sum([np.sum(W*W) for W in [W1, W2, W3]])
  loss = data_loss + reg_loss
...
def init_three_layer_convnet(weight_scale=1e-3, bias_scale=0, input_shape=(3, 32, 32),
                           num_classes=10, num_filters=(32,128), filter_size=5):
  C,H,W = input_shape
  assert filter_size % 2 == 1, "Filter size must be odd; got %d" % fitler_size
  model = {}
  model['W1'] = weight_scale * np.random.randn(num_filters[0],C,filter_size,filter_size)
  model['b1'] = bias_scale*np.random.randn(num_filters[0])
  model['W2'] = weight_scale * np.random.randn(num_filters[0]*H*W/4, num_filters[1])
  model['b2'] = bias_scale*np.random.randn(num_filters[1])
  model['W3'] = weight_scale*np.random.randn(num_filters[1],num_classes)
